...
public class Operations {
    ...
    public static void task4(List<InfoCompany> companies) {
        ...
        if (currency.equalsIgnoreCase("rub")
            ...
            List<Security> securitiesUsingCurrency = companies.stream().map(InfoCompany::getSecurities).flatMap(Collection::stream)
...
//					.forEach(i -> System.out.printf("%s (%s)%n", i.getCode(), i.getName()));
    ...
    public static void task3(List<InfoCompany> companies) {
    ...
    public static void task2(List<InfoCompany> companies) {
        System.out.println("\nExpired securities (code - name - expires):");
        List<Security> expired = companies.stream().map(InfoCompany::getSecurities).flatMap(Collection::stream)
    ...
    public static void task1(List<InfoCompany> companies) {
    ...
    public static Companyes parseJson(String fileName) {
        ...
        Companyes companies = null;
        try {
            companies = mapper.readValue(new File(fileName), Companyes.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return companies;
